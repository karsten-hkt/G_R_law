...
#读取数据
data_locate = '../out_file/b_variation_after_7_8/data_processing_7_8.csv'
...
para = {'day_line':0, 'lon_line':2, 'lat_line':3, 'mag_line': 5, 'delta_M':0.1, 'roi':[34,41,35,40],
        'delta_time':10, 'res_ratio':0.1, 'distance':0.2, 'calculate_min_number':50}
...
out_gmt_locate = '../out_file/b_variation_after_7_8/'
X_len = round((para['roi'][1]-para['roi'][0])/para['res_ratio'])
Y_len = round((para['roi'][3]-para['roi'][2])/para['res_ratio'])
time_circle_number = 5#np.ceil(max(data_numpy[:,para['day_line']])/para['delta_time'])
...
for tmp_time in range(round(time_circle_number)):
    ...
    ## 是否保存需要的计算过程中的文件
    if save_file == True:
        np.savecsv('../out_file/b_variation_after_7_8/b_record_%s.txt'%tmp_time,
                   b_record, fmt='%.4f',newline='\n')
        np.savetxt('../out_file/b_variation_after_7_8/Mc_record_%s.txt'%tmp_time,
                   Mc_record, fmt='%.4f',newline='\n')
        np.savetxt('../out_file/b_variation_after_7_8/p_record_%s.txt'%tmp_time,
                   p_record, fmt='%.4f',newline='\n')
        np.savetxt('../out_file/b_variation_after_7_8/b_error_record_%s.txt'%tmp_time,
