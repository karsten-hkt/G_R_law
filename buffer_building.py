...
aftershock_file = '../data/data_processing.csv'
...
for i in range(len(outdata)):
    ...
    np.savetxt('../out_file/buffer_generate/buffer_test_%d.txt' % i,out_buffer,fmt='%f,%f',newline='\n')
    data = pd.DataFrame(out_aftershocks)
    data.to_csv('../out_file/buffer_generate/aftershocks_in_buffer_%d.csv' % i,index=0,header=0)
    print('success')
plt.savefig('../out_file/buffer_generate/buffer_shape.png',bbox_inches='tight')
