...
b_1, b_error_1, Mc_1 = b_cal.month_b_value_calculate_and_draw(data_numpy[(data_numpy[:,0]<30)],
                                                              minnum, delta_M, mag_line, color='red')
b_2, b_error_2, Mc_2 = b_cal.month_b_value_calculate_and_draw(data_numpy[(data_numpy[:,0]<60) & (data_numpy[:,0]>=30)],
                                                              minnum, delta_M, mag_line, color='orange')
b_3, b_error_3, Mc_3 = b_cal.month_b_value_calculate_and_draw(data_numpy[(data_numpy[:,0]>=60) & (data_numpy[:,0]<90)],
                                                              minnum, delta_M, mag_line, color='m')
b_4, b_error_4, Mc_4 = b_cal.month_b_value_calculate_and_draw(data_numpy[(data_numpy[:,0]>=90)],
                                                              minnum, delta_M, mag_line, color='g')
plt.yscale("log")
labels = ['from 02/06 to 03/07','from 03/08 to 04/06','from 04/07 to 05/06','from 05/07 to 05/31']
...
plt.savefig('../out_file/month_file/turkey_b_month.png',bbox_inches='tight')
plt.savefig('../out_file/month_file/turkey_b_month.pdf',bbox_inches='tight')