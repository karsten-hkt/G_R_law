...
for tmp_time in range(round(time_circle_number)):
    ...
    ## 是否保存需要的计算过程中的文件
    if save_file == True:
        np.savecsv('../data/b_spatial_temproal_variation/b_record_%s.txt'%tmp_time,
                   b_record, fmt='%.4f',newline='\n')
        np.savetxt('../data/b_spatial_temproal_variation/Mc_record_%s.txt'%tmp_time,
                   Mc_record, fmt='%.4f',newline='\n')
        np.savetxt('../data/b_spatial_temproal_variation/p_record_%s.txt'%tmp_time,
                   p_record, fmt='%.4f',newline='\n')
        np.savetxt('../data/b_spatial_temproal_variation/b_error_record_%s.txt'%tmp_time,
