...
def ts_infs2apsA(datainfo):
    ...
    for ni in range(datainfo.shape[0]):
        ...
        A_con[ni,m_indx[0]] = -1.
        A_con[ni,s_indx[0]] = 1.
        pairMatrix[ni,:] = [m_indx,s_indx,diffdays]
    #
...
def yyyymmdd2jd(yyyymmdd):
    return timestr2jd(str(int(yyyymmdd)),fmt='%Y%m%d')
    #
...
def orbitalCor(phs,mlon,mlat,dem='NULL',thirddep='NULL',\
               istheilsen=True,scale=1,npara=3,cc=None,demexp=False):
    '''
