...
def updateboundary_APS(updated_phs,aps,apsdates,apsarray):
    ...
    return aps
#
def commonAPS(updated_phs,cosd,info,udates,iters,initers=2,maxpairs=2,mode='pre'):
    #
    ...
    #
    if mode.upper() == 'POST':
       cphs = updated_phs[updated_phs[:,0]>cosd,:]
    else:
       if mode.upper() == 'PRE':
           cphs =  updated_phs[updated_phs[:,1]<cosd,:]
       else:
    ...
    indays = np.array([diff2dates(cosd,dates[i]) for i in range(dates.shape[0])])
    #
    apsarray = None
    A1       = None
    pairIDs  = None
    ...
    while (tmpiters > 0):
      ...
      #
      if True:
         x,res,rnk,info = np.linalg.lstsq(A,tmpphs[:,2],rcond=None)
      ...
      #
      # print(tmpiters,iters,totalaps[3])
      tmpiters = tmpiters - 1
      counter = counter + 1
    #
...
def dates2ts(udates):
    ...
    return ts_time
#
def diff2dates(dt1,dt2):
    #
    cdt1 = datetime.datetime.strptime(str(int(dt1)),'%Y%m%d')
    cdt2 = datetime.datetime.strptime(str(int(dt2)),'%Y%m%d')
    dt = cdt2 - cdt1
    # 
    return dt.days
#
...
#
def aps(data,cosd,iters,maxpairs=2,isplot=False,unit='mm',A=None,mdays=None,apsplot=False,cosp=False,freqinfo=False):
    #
    ...
    #
    if cosd is not None:
        cosd_dt = diff2dates(alldates[0],cosd) 
    #
    ...
    #
    preaps,predates = commonAPS(updated_phs,cosd,info,udates,iters,maxpairs=maxpairs,mode='pre')
    updated_phs = update_infs(data,preaps,predates)
    #
    postphs = updated_phs[updated_phs[:,0]>cosd,:]
    #
    if postphs.shape[0]>0:
      # 
      post_udates = np.sort(np.unique(postphs[:,0:2]))
      postaps,postdates = commonAPS(postphs,cosd,info,post_udates,iters,maxpairs=maxpairs,mode='post')
      updated_post_phs = update_infs(postphs,postaps,postdates)
      # 
      postA,postlap,postmdays,postdates = infs2A(updated_post_phs)
      #
      postdays = [diff2dates(cosd,postdates[i]) for i in range(postdates.shape[0])]
      # 
      # removing a log post-trend 
      #
      goFlag = False
      if goFlag:
         post_x,post_res,post_rnk,post_info = np.linalg.lstsq(postA,updated_post_phs[:,2],rcond=None)
         postdis = x2accumulateddis(post_x)
         #
         paras = log_fit(postdays[1::],postdis)
         trend_post = log_func(postdays[1::],paras[0],paras[1],paras[2])
         #
         nodef_post_phs = removepost(postphs,postdates[1::],trend_post)
         #
         # post_x_1,post_res,post_rnk,post_info = np.linalg.lstsq(postA,tmpphs[:,2],rcond=None)
         # postdis_1 = x2accumulateddis(post_x_1)
         #
         postaps,postdates = commonAPS(nodef_post_phs,cosd,info,post_udates,iters,maxpairs=maxpairs,mode='post')
         updated_post_phs = update_infs(postphs,postaps,postdates)
         #
         updated_post_x,post_res,post_rnk,post_info = np.linalg.lstsq(postA,updated_post_phs[:,2],rcond=None)
         updated_postdis = x2accumulateddis(updated_post_x)
      #
      checking = False
      #
      if checking:
        plt.plot(postdays[1::],postdis,'-or')
        plt.plot(postdays[1::],updated_postdis,'-ok')
        # plt.plot(postdays[1::],postdis_1,'-og')
        plt.plot(postdays[1::],trend_post,'-b',linewidth=3.5)
        plt.show()
        sys.exit(-1)
      #
    else:
    ...
    if cosp:
       #
       #
       cosd_dt = pSAR.ts.dates2dates(np.array([cosd]))
       ymin,ymax = plt.gca().get_ylim()
       # cos_data = np.zeros([2,2])
       #
       if cosp:
         ax = plt.gca()
         ax.arrow(cosd_dt[0],ymax, 0, -1* (ymin+ymax)/2, width=3.5,label='Coseismic')
