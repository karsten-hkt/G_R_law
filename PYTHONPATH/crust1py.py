...
class crustModel:
    ...
    def __init__(self):
        # Read in data files
        # added by FWP, @SYSU, 2021/06/13
        #
        datadir = '/home/wafeng/soft/seis/crust1.0_python/'
        if not os.path.exists(datadir):
            #
            print(" Error: cannot find the preset path of %s" % datadir)
            sys.exit(-1)
        #
        self.vp = np.loadtxt(datadir+'crust1.vp')
        self.vs = np.loadtxt(datadir+'crust1.vs')
        self.rho = np.loadtxt(datadir+'crust1.rho')
        self.bnds = np.loadtxt(datadir+'crust1.bnds')
