...
para = {'day_line':0, 'lon_line':2, 'lat_line':3, 'dep_line':4, 'mag_line': 5, 'delta_M':0.1,
        ...
        'drawing_number':5, 'buffer_length': 30000, 'depth':20}
#主震
...
for tmp_time in range(para['drawing_number']):
    ...
    for tmp_fault in range(len(fault)):
        ...
        outx, outy = LocT.simp_rotaxll(out_aftershocks[:, para['lon_line']], out_aftershocks[:, para['lat_line']],
                                       fault.iloc[tmp_fault, 5], fault.iloc[tmp_fault, 1], fault.iloc[tmp_fault, 2])
        ...
        for i in range(refpts.shape[0]):
            b_value[i], b_error[i] = b_cal.eqs2bINpoly_maxc(out_aftershocks, refpts_poly[i],
                                                            ...
                                                            refpolys[i], tmp_time)
        if tmp_fault == 0:
            area_cut.ref2gmt_3d(refpolys, b_value, fault.iloc[tmp_fault, 5], fault.iloc[tmp_fault, 1],
                                fault.iloc[tmp_fault, 2], outgmt_name, tmp_time, out_gmt_locate)
        else:
            area_cut.ref2gmt_3d_exist(refpolys, b_value, fault.iloc[tmp_fault, 5], fault.iloc[tmp_fault, 1],
                                fault.iloc[tmp_fault, 2], outgmt_name, tmp_time, out_gmt_locate)
