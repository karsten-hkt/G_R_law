...
aftershock_file = '../data/zhou_data_processing.csv'
...
for i in range(1):
    ...
    cdata = cpolygon[[0,1],:]
    np.savetxt('../out_file/zhou_result/zhou_Prof_test_%d.inp' % i,cdata,fmt='%f %f',newline='\n')
plt.scatter(aftershocks[:,3],aftershocks[:,2])#,s = aftershocks[:,5]/10)
...
for i in range(len(outdata)):
    ...
    for j in range(aftershocks.shape[0]):
        point = np.zeros(2)
        point[0] = aftershocks[j,3]
        point[1] = aftershocks[j,2]
        if area_cut.is_in_poly(point,out_buffer):
            out_aftershocks.append(aftershocks[j,:])
    np.savetxt('../out_file/zhou_result/zhou_buffer_test_%d.txt' % i,out_buffer,fmt='%f,%f',newline='\n')
    data = pd.DataFrame(out_aftershocks)
    data.to_csv('../out_file/zhou_result/zhou_aftershocks_in_buffer_%d.csv' % i,index=0,header=0)
    print('success')
plt.savefig('../out_file/zhou_result/zhou_buffer_shape.png',bbox_inches='tight')
